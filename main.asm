...
#maze:   .space 400       # 30*40*4 = 4800 bytes
maze: 	
...
main:
    ...
    addi $s0, $zero, 16 # preciseBallX aka the true X value multiplied by 16
    addi $s1, $zero, 16 # preciseBallY aka the true Y value multiplied by 16
...
inner_maze_display_loop:
	...
	mul  $s7, $a2, $s3     # $t4 = i * k (row offset)
    	add  $s7, $s7, $a1     # index = (i*k) + j = full 1D index
...
collision_case3: # char 3 == slow debuff
	subi $s4, $zero, 1 # set speed buff/debuff val to -1
	
	addi $a0, $zero, 0
	srl $a1, $s2, 4 # divide preciseBallX by 16 to revert back to screen units
	srl $a2, $s3, 4 # divide preciseBallY by 16 to revert back to screen units
	
	jal putChar_atXY
	
	j resume_after_collisions
	
collision_case4: # char 4 == super speed
...
use_wallbreaker_on_damaged_wall:
	...
	add $a0, $zero, 0
	srl $a1, $s2, 4 # divide preciseBallX by 16 to revert back to screen units
	srl $a2, $s3, 4 # divide preciseBallXY by 16 to revert back to screen units
...
ball_x_too_high:
	...
	add $v0, $zero, $t3
	subi $v0, $v0, 1
	sll $v0, $v0, 4 # restore the return value to the correct units
...
ball_y_too_high:
	...
	add $v1, $zero, $t2
	subi $v1, $v1, 1
	sll $v1, $v1, 4 # restore the return value to the correct units
